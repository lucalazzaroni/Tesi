...
\begin{figure}[ht]
	\centering
	\includegraphics[scale=0.5]{braccioForza.PNG}
...
Poiché nel nostro caso $r$ risulta essere il raggio della ruota ed F la forza tangente alla ruota abbiamo ricavato $$F=\displaystyle\frac{\tau}{r}\Rightarrow C=\displaystyle\frac{1}{r}=29.41$$ 
\begin{figure}[ht]
	\centering
...
Attraverso l'utilizzo di uno strumento MATLAB
\begin{figure}[ht]
	\centering
...
\end{figure}
 quale `sisotool' siamo arrivati a definirne il guadagno $P=-2$, in seguito verificato anche sperimentalmente.\\
Nella realizzazione dello schema a blocchi in Simulink, caricato nel Brick EV3 MINDSTORM per confermare la veridicità dei risultati ottenuti in MATLAB,
\begin{figure}[ht]
	\centering
	...
	\label{pendoloReale}
\end{figure}
abbiamo posto un ingresso costante 0 poiché si assume il pendolo inizialmente a riposo nel punto di equilibrio $\theta=\ang{0}$.\\
...
Alla variare della posizione angolare, il motore reagisce muovendo il carrello nella stessa direzione per riportare il 
\begin{figure}[ht]
	\centering
	...
	\label{oscillOL}
\end{figure} 
pendolo nel suo punto di equilibrio.\\
