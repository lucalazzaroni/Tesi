...
Per farlo ci siamo serviti del `Linear Analisys Tool' di Simulink che, tra le altre cose, può essere usato per ricostruire la funzione di trasferimento tra due punti specificati i un sistema (nel nostro caso tra l'ingresso del motore e l'uscita $\theta$).
$$T_{y_2,u}=-\displaystyle\frac{559.5s+78.3}{s^4+31.67s^3+227s^2+1910s+10050}$$\\
Tra i principali modi per studiarne la stabilità in ciclo chiuso c'è sicuramente il luogo delle radici che permette di verificare lo spostamento dei poli al variare del guadagno.
\newpage
